//! DLG1 Rename Window Dialog (PANE-08 + DLG-02 per D6).
//!
//! Modal rename built on `gpui-component` Dialog + InputState, cloning the
//! DLG1 lifetime from `rename_session_dialog.rs`: a fresh `RenameWindowForm`
//! entity per open, `None` on dismiss, `is_submitting` double-submit guard
//! (T-03-07), inline `#7F1D1D` error.
//!
//! Prefill is `w.name` (FE `WindowTabs.tsx:154` parity); the gate is trim +
//! non-empty only (no session-name validation — tmux names accept anything).
//! Submit sends correlated `window.rename` (`name` travels as a JSON field
//! into argv — no shell, T-05-04) with dialog feedback: success clears the
//! form + closes, `command.error`/timeout renders inline with the dialog
//! open. The window target rides `paneId` with the `@N` id (no `windowId`
...
/// on re-open.
pub struct RenameWindowForm {
    /// Window being renamed (`@N`).
    pub target: String,
...
pub fn open_rename_window_dialog(
    ...
    // Prefill with `w.name` + autofocus (FE parity).
    ...
    let form = cx.new(|_cx| RenameWindowForm {
        target: target_id,
    ...
    let sub = {
        ...
        cx.subscribe(&name_input, move |_: Entity<InputState>, event, cx| match event {
            InputEvent::PressEnter { .. } => request_rename_window_submit(&form_weak, cx),
            _ => {}
        })
    };
    ...
    window.open_dialog(cx, move |dialog, _window, cx| {
        dialog
            ...
            .child(render_rename_window_body(&build_form, cx))
            .footer(render_rename_window_footer(&build_form, cx))
            .on_close({
    ...
    app_entity.update(cx, |app, cx| {
        app.rename_window_form = Some(form);
        cx.notify();
    });
}
...
fn render_rename_window_body(form: &Entity<RenameWindowForm>, cx: &mut App) -> impl IntoElement {
    ...
    body = body.child(
        div()
            ...
            .child(format!("Rename window \"{target}\" to a new name.")),
    );
...
fn render_rename_window_footer(form: &Entity<RenameWindowForm>, cx: &mut App) -> DialogFooter {
    let foreground_text = rgb(0xd4d4d4);
                ...
                .when(!is_submitting && !name_empty, |s| {
                    s.cursor_pointer()
                        ...
                        .on_mouse_down(MouseButton::Left, {
                            let form_weak = form.downgrade();
                            move |_, _, cx| {
                                request_rename_window_submit(&form_weak, cx);
                            }
                        })
                })
                .child("Rename"),
...
/// Trim + non-empty gate, then a correlated `window.rename` with dialog
...
pub fn request_rename_window_submit(form: &WeakEntity<RenameWindowForm>, cx: &mut App) {
    ...
    let msg = AppState::build_window_rename(&target, &name);
